import { Box, Button, Flex, FormControl, FormErrorMessage, FormLabel, Heading, Input, Stack, StackDivider, Text, Textarea, useColorModeValue, useDisclosure } from '@chakra-ui/react';
...
function Order() {
  ...
  const updateOrderForm = useFormik({
    ...
    initialValues: {
      ...
      allDay: order?.allDay ? { label: "Ja", value: true } : { label: "Nej", value: false },
      reccuring: orderReccurencyOptions.find(x => x.value == order.recurrencyInterval?.id),
      customer: {label: order?.customer?.name, value: order?.customer?.id},
      employees: order?.users?.map(u => ({label: u.firstName + " " + u.lastName, value: u.id})),
    ...
    validationSchema: Yup.object({
      ...
      start: Yup.string("Måste vara datum & tid.").required("Start tid krävs."),
        ...
        .test("is-greater", "Slut tid måste vara efter start tid", function (value) {
  ...
  const init = async () => {
    const getRequiredFormData = async () => {
      ...
      setRequiredFormData({ customers: responseCustomers.data.data, teams: responseTeams.data.data, employees: responseEmployees.data.data })
    }
              ...
              <Stack spacing="4" divider={<StackDivider />}>
                <FormikProvider value={updateOrderForm}>
                  <FieldGroup title={"Allmänt"}>
                    <FormControl isInvalid={updateOrderForm.errors.title && updateOrderForm.touched.title}>
                      ...
                      <FormErrorMessage>{updateOrderForm.errors.title}</FormErrorMessage>
                    </FormControl>
                  </FieldGroup>

                  <FieldGroup title={"Tid"}>
                    <Stack w="full">
                      <FormControl isInvalid={updateOrderForm.errors.start && updateOrderForm.touched.start}>
                        ...
                        <FormErrorMessage>{updateOrderForm.errors.start}</FormErrorMessage>
                      </FormControl>

                      <FormControl isInvalid={updateOrderForm.errors.end && updateOrderForm.touched.end}>
                        ...
                        <FormErrorMessage>{updateOrderForm.errors.end}</FormErrorMessage>
                      </FormControl>

                      <FormControl isInvalid={updateOrderForm.errors.allDay?.label}>
                        ...
                        <Select
                          ...
                          onChange={o => updateOrderForm.setFieldValue("allDay", o)}
                      ...
                      <FormControl>
                        ...
                        <Select
                          ...
                          onChange={o => updateOrderForm.setFieldValue("reccuring", o)}
                  ...
                  <FieldGroup title={"Kund, Team & Anställda"}>
                    <Stack w="full">
                      <FormControl isInvalid={updateOrderForm.errors.customer?.label}>
                        ...
                        <Select
                          ...
                          onChange={o => updateOrderForm.setFieldValue("customer", o)}
                          options={requiredFormData.customers.map(c => ({ label: c.name, value: c.id }))}
                        />
                        <FormErrorMessage>{updateOrderForm.errors.customer?.label}</FormErrorMessage>
                      </FormControl>
                      <FormControl>
                        ...
                        <Select
                          ...
                          isMulti
                          focusBorderColor={"brand.primary"}
                          onChange={o => updateOrderForm.setFieldValue("employees", o)}
                      ...
                      <FormControl>
                        ...
                        <Select
                          ...
                          isMulti
                          focusBorderColor={"brand.primary"}
                          onChange={o => updateOrderForm.setFieldValue("teams", o)}
                  ...
                  <FieldGroup title={"Övrigt"}>
                    <Stack w="full">
                      <FormControl isInvalid={updateOrderForm.errors.notes && updateOrderForm.touched.notes}>
                        ...
                        <FormErrorMessage>{updateOrderForm.errors.notes}</FormErrorMessage>
                      </FormControl>
                      <FormControl>
                  ...
                  <FieldGroup title={"Spara"}>
                    <Stack w="full">
                    <Button onClick={updateOrderForm.handleSubmit} w="full" colorScheme="brand">Spara ändringar</Button>
                    <Text opacity={0.5} pt="1">Notera att dessa ändringar bara tillämpas när det skapas nya bokningar av denna arbetsordern.</Text>
                    </Stack>
                  </FieldGroup>
                </FormikProvider>
                <FieldGroup title={"Ta bort"}>
                  <Button onClick={onOpen} w="full" colorScheme={"red"}>Ta bort arbetsorder</Button>
                </FieldGroup>
              </Stack>
            </Box>
          </Box>
