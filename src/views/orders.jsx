import { Flex, Heading, Button, Box, useColorModeValue, Text, Stack, Center, Accordion, AccordionItem, AccordionButton, AccordionIcon, AccordionPanel, FormControl, FormLabel, Input, IconButton, useDisclosure } from '@chakra-ui/react'
    ...
    <>
      <Flex direction="column" flex="1" overflow="auto" px="10" pt="8">
        <Flex align="center" justify="space-between">
          ...
          <Link to="/orders/new">
          <IconButton mt="2.5" icon={<HiPlus />} text="text" mb="6" size="md" /></Link>
        </Flex>
        <Flex>
                    ...
                    <AccordionPanel bg="whiteAlpha.800" pb={4}>
                      <Stack>
                        <FormControl>
                          ...
                          <Input name="name" value={searchFilter.name} onChange={(e) => setSearchFilter({ ...searchFilter, name: e.target.value })} placeholder="Sök på namnet" />
                        </FormControl>
                        <FormControl>
                          <FormLabel>Kund</FormLabel>
                          <Select
                            value={searchFilter.customer}
                            onChange={(e) => setSearchFilter({ ...searchFilter, customer: e })}
                        ...
                        <FormControl>
                          ...
                          <Select
                            ...
                            onChange={(e) => setSearchFilter({ ...searchFilter, recurring: e })}
                            menuPortalTarget={document.body}
                            menuPosition={'fixed'}
                            styles={{ zIndex: 1000 }}
                            name="reccurency"
                        ...
                        <Button bg="brand.primary" textColor="white" onClick={() => { getOrders() }}>sök</Button>
                ...
                {isLoading ?
                  ...
                  <>
                    {orders.data.length < 1 && (
                      <Box colSpan="4">
                        <Center>
                          <Text>Inga arbetsordrar hittades, testa ändra ditt filter eller lägg till en ny arbetsoder.</Text>
                        </Center>
                      </Box>
                    )}
                    ...
                    <Stack spacing={3}>
                      {orders.data?.map(order => (
                        <OrderCard openDeleteModal={openDeleteModal} order={order} />
                      ))}
      ...
      <OrderAddForm getOrders={getOrders} isOpen={addIsOpen} onOpen={addOnOpen} onClose={addOnClose} />
      <OrderDelete deleteOrder={deleteOrder} isOpen={deleteIsOpen} onClose={deleteOnClose} onOpen={deleteOnOpen} />
    </>
  )
}
