import { Select, Flex, Heading, Box, Text, Button, Card, Input } from "@chakra-ui/react";
...
function InspectionCard(props) {
   ...
   /* useEffect(() => {
    ...
    const addItem = (listIndex, item) => {
        ...
        const existingItemIndex = newLists[listIndex].items.findIndex(i => i.id === item.id)
    ...
    const removeItem = (listIndex, itemIndex) => {
        const newLists = [...lists];
        const foundObj = products.find(el => el.id === newLists[listIndex].items[itemIndex].id);
    ...
    const addd = (list, id) => {
        ...
        if (foundObj) {
            addItem(list, { id: foundObj.id, name: foundObj.name, volume: foundObj.volym, count: 1 })
        } else {
            return null;
        }
    }
    ...
    const EditInspection = () => {
        const data = { Id:Eid,InspectionName: inspectionName, InspectionData: lists };
    ...
    return (
        <>
            ...
            {select && <><Select  onChange={(e) => setSelected(e.target.value)}>
                <option value="">Select a product</option>
                {inSpecList && inSpecList.map((is, i) => (
                    <option value={is.Id}>{is.Id} {is.Name}</option>
                ))}
                </Select></> }
            <Input value={inspectionName} onChange={(e) => setInspectionName(e.target.value)} type='text' />
            <Box flex={"1"}>
                ...
                <Flex>
                    {lists?.map((list, index) => (
                        <Card p={4} m={3} key={index}>
                            <h2>{list.name}<Button onClick={() => (removeList(index))}>Ta bort rum</Button></h2>
                            {list.items.map((item, itemIndex) => (

                                <Card p="3" m="1" key={itemIndex}>
                                    {item.name} - {item.volume} m3 . Antal {item.count}
                                    <Button onClick={() => removeItem(index, itemIndex)}>Ta bort</Button>
                                </Card>
                            ))}
                            <p>Total volym: {list.items.reduce((acc, item) => acc + item.volume, 0)} m3</p>
                            ...
                            <Button disabled={!selectedProductId} onClick={() => { addd(index, selectedProductId) }}>Lägg till</Button>
                        </Card>
                    ))}
                </Flex>
