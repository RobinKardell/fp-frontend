...
import { List, ListItem, ListIcon, Modal, ModalContent, ModalHeader, ModalOverlay, ModalBody, Box, Text, Input, FormControl, FormLabel,Button } from "@chakra-ui/react";
...
function EditArticleRow(props) {
    const { rowData, setOrderRows, ordersRows, isOpen, onOpen, onClose, articels } = props;
    ...
    const addToList = () => {
        ...
        const totalT = (rowData.type === "h") ? (((timeDifference - pbreak) * price) * count) : price * count
    
        
        // Find the index of the item to update
const index = ordersRows.findIndex((row) => row.artnr === rowData.artikelnr);
...
const newRows = [...ordersRows.slice(0, index), updatedItem, ...ordersRows.slice(index + 1)];
        ...
        <Modal scrollBehavior={"inside"} isCentered isOpen={isOpen} onClose={onClose}>
            <ModalOverlay>
                <ModalContent borderRadius={0}>
                    <ModalHeader borderBottom={"1px"}>Ändra vald artikelrad</ModalHeader>
                    <ModalBody>
                        {rowData &&(
                            <>
                                <Box>
                                    ...
                                    <FormControl>
                                        ...
                                        <Input border="1px solid black" name="category" defaultValue={rowData.category} readOnly />
                                    </FormControl>
                                    <FormControl>
                                    ...
                                    <FormControl>
                                        ...
                                        <Input border="1px solid black" name="hourprice" value={price} />
                                    </FormControl>
                                </Box>
