...
function AddArticleRow(props) {
  const { setOrderRows, ordersRows, isOpen, onOpen, onClose } = props;
  ...
  const addToList = () => {
    ...
    const totalT =
      selectedProduct.type === "h"
        ? (timeDifference - pbreak) * selectedProduct.price * count
                ...
                {isOpen1 && filteredProducts.length > 0 && (
                    ...
                    <List spacing={0}>
                        ...
                        >
                            ...
                            />
                            <Text>
                            #{product.artikelnr} : {product.name}
                            </Text>
                        </ListItem>
                        ))}
                    </List>
                      ...
                      return (
                        <>
                          <ListItem
                            my={"5px"}
                            key={index}
                            onClick={() => handleProductSelect(articel)}
                          >
                            <Text
                              backgroundColor={
                                articel === selectedProduct
                                  ? "gray.300"
                                  : "white"
                              }
                            >
                              #{articel.artikelnr} : {articel.name}
                            </Text>
                          </ListItem>
                        </>
                      );
